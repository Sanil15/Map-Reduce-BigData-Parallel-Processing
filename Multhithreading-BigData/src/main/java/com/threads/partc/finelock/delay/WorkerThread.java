...
public class WorkerThread implements Runnable {
	...
	// Otherwise if Station Id exists it makes a delayed(Invoking Fibonnaci of 17 first) update  
	// to map's entry by accumulating temperature values and incrementing the record count for that station. 
	// While these operations the Thread holds a lock on Station Object
	@Override
	public void run() {
		for(String a: records){
			...
			if(stationDetail[2].equals("TMAX")){
				...
				if(CalculateTMAXFineLockWithDelay.stationMap.containsKey(stationDetail[0])){
					Station obj = CalculateTMAXFineLockWithDelay.stationMap.get(stationDetail[0]);
					// a Synchronized method in Station Object
					obj.setAttributesWithDelay(obj.getTmaxTotal()+Double.parseDouble(stationDetail[3]), obj.getRecordCount()+1);
					...
					synchronized (objNew) {
						...
						CalculateTMAXFineLock.stationMap.put(stationDetail[0],objNew);	
							...
							objExisting.setAttributesWithDelay(objExisting.getTmaxTotal()+objNew.getTmaxTotal(), objExisting.getRecordCount()+1);
