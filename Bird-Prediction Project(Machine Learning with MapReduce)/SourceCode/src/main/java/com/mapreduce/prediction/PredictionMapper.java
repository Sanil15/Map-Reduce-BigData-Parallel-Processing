...
/** PredictionMapper works on the unlabled data
...
public class PredictionMapper extends Mapper<Object, Text, Text, Text> {
	...
	public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
		...
		String checklistFormatted = getChecklist(checklist,sampling_event_id);
		...
		String []checklistColums = checklistFormatted.split(",");
		...
		// emit the record with sampling id as key
		if(!locationId.trim().isEmpty())
			context.write(new Text(sampling_event_id), new Text(output));
	}
	
	/** HELPER METHODS TO CLEAN, EXTRACT features of CHECKLIST, CORE and EXTENDED, and represent them in sparse format */
	...
	public String getChecklist(String checklist, String sampling_event_id){
		...
		Long yearId = null;
		/*
		if(yearMapping.containsKey(yearVal)){
			yearId = yearMapping.get(yearVal);
		}
		else{	
			yearMapping.put(yearVal, yearCount++);
			yearId = yearMapping.get(yearVal);
		}
		 */
		...
		//		Long timeValLong = Math.round(timeVal) % 24;
		...
		String agelaius_phoeniceus = checklistColums[26]; // is equal to ?
		...
		sb.append("0:"+sampling_event_id);


		/*	if(!locationId.trim().isEmpty()){
			sb.append(",1:"+locationId);
		}*/
